...
export default class ChexManager {
  ...
  _renderSceneControls(controls) {
    const v13 = game.release.generation == 13;

    if (v13) {
      this._extendSceneControlButtonsV13(controls);
    } else {
      this._extendSceneControlButtonsV12(controls);
    }
  }

  _extendSceneControlButtonsV12(buttons) {
        ...
        toolBox.tools.push(
          {
            ...
            order: 3,
            onClick: async () => {
              if (chex.realmSelector) chex.realmSelector.close();
              if (!chex.terrainSelector) new TerrainPalette().render(true);
            },
          },
          {
            ...
            order: 4,
            onClick: async () => {
              if (chex.terrainSelector) chex.terrainSelector.close();
              if (!chex.realmSelector) new RealmPalette().render(true);
            },
          }
        );
      }
  ...
  _extendSceneControlButtonsV13(controls) {
    const tokenTools = controls.tokens?.tools;

    var registerTool = (tools, tool) => {
      // tools need to be registered such that name matches the variable
    ...
    if (this.active) {
      ...
      this.#showTerrainTool = {
        ...
        button: false,
        visible: true,
        order: Object.keys(tokenTools).length + 2,
        active: this.mode === C.MODE_TERRAIN,
        onChange: () => {
          this.mode = C.MODE_TERRAIN;
          this.#showKingdomTool.active = false;
      ...
      this.#showTravelTool = {
        ...
        button: false,
        visible: true,
        order: Object.keys(tokenTools).length + 3,
        active: this.mode === C.MODE_TRAVEL,
        onChange: () => {
          this.mode = C.MODE_TRAVEL;
          this.#showKingdomTool.active = false;
      ...
      registerTool(tokenTools, this.#showChexDetailsTool);
      registerTool(tokenTools, this.#showKingdomTool);
      registerTool(tokenTools, this.#showTerrainTool);
      registerTool(tokenTools, this.#showTravelTool);
    }
    ...
    if (game.user.isGM) {
      const toolBox = {
        ...
        onChange: () => {}, // do nothing
        onToolChange: () => {}, // do nothing
      };

      if (this.active) {
        var chexTerrainSelector = {
          name: "chexTerrainSelector",
          title: "CHEX.TOOLS.TerrainSelector",
          icon: "fa-solid fa-mountain",
          button: true,
        ...
        registerTool(toolBox.tools, chexTerrainSelector);
        registerTool(toolBox.tools, chexRealmSelector);
      }
      // Also register with a matching name
      registerTool(controls, toolBox);
    }
  }

  get isValidGrid() {
    return (
      canvas.grid.type === foundry.CONST.GRID_TYPES.HEXODDR ||
      canvas.grid.type === foundry.CONST.GRID_TYPES.HEXODDQ
    );
  }
  ...
  #refreshKingdomLayer() {
    if (this.kingdomLayer) this.kingdomLayer.draw();
  }
    ...
    if (this.active) {
      ...
      if (this.isValidGrid) {
        ...
        ui.notifications.warn(
          "Currently, Chex only works for Hexagonal Rows - Odd, or Hexagonal Columns - Odd"
        );
      }
    }
  ...
  _onReady() {
    ...
    if (!this.kingdomLayer) {
      ...
      if (canvas.scene.tokenVision)
        canvas.stage.rendered.environment.effects.addChildAt(
          chex.manager.kingdomLayer,
          1
        );
      else canvas.interface.grid.addChild(this.kingdomLayer);
    }
    this.kingdomLayer.draw();
  ...
  #onMouseDown(event) {
    ...
    if (!this.hud.enabled || !this.hoveredHex) return;
  ...
  #paintTerrainDeferredEnd() {
    if (this.pendingPatches.length > 0) {
      const patches = this.pendingPatches.reduce(
        (result, item) => {
